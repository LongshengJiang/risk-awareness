...
class Robot:
    ...
    def __init__(self, robot_id, region_prob):
        ...
        self.p_o1_s1 = 0.5  # Probability of true positive.
        self.p_o1_s0 = 0.5  # Probability of false positive.
    ...
    def confirm(self):
        ...
        # We will only confirm the observation if the probability of the observation to be correct is greater than 9.0
        # and the cell is not visited.
        if self.post_prob >= 9.0 and not self.visited_cell[self.current]:
...
class Human:
    ...
    def __init__(self, num_history_services):
        ...
        self.hm_tsk_dur = 10  # human task duration is several times slower than that of the robot detection.
...
def main():
    ...
    while time_step < total_time_step and total_found_tar < mis_tar:
        ...
        # We will only consider the robots whose detection correctness probability >= 9.0 to be in the waiting line.
        active_robots = []
        # We will record the robots that are released before the robot ordering.
        released_robots_pre = []
        for active_candi in active_candi_robots:
            if active_candi[1] >= 9.0:
...
if __name__ == '__main__':
    ...
    filename = './results/condition14_RTx_0505.txt'
