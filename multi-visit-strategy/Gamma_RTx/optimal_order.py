    ...
    def _line_length(self, robot_list):
        ...
        while True:
            ...
            for robot in robot_list:
                ...
                # convenience.
                #               q = 1.456 * delta_u + 0.03015 * sinh(10 * delta_u);
                #               w = 0.0394 + 0.9476 * exp(-0.1668 * (-math.log(prob)) ^ 1.107);
                ...
                q_func = lambda delta_u: 1.456 * delta_u + 0.03015 * math.sinh(10 * delta_u)
                q_value_correct = q_func(delta_u_correct)
                q_value_wrong = q_func(delta_u_wrong)
                # Let us now focus on the w-function
                w_func = lambda prob: 0.0394 + 0.9476 * math.exp(-0.1668 * (-math.log(prob)) ** 1.107)
                if pr == 0.0:
                    w_value = 0.0
                elif pr == 1.0:
                    w_value = 1.0
