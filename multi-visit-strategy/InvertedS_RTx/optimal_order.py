    ...
    def _line_length(self, robot_list):
        ...
        while True:
            ...
            for robot in robot_list:
                ...
                # convenience.
                #               q = 1.871 * delta_u + 0.002686 * sinh(9.428 * delta_u);
                #               w = 0.1782 + 0.7186 .* exp(-1.033 .* (-log(p)) .^ 0.3705);
                ...
                q_func = lambda delta_u: 1.871 * delta_u + 0.002686 * math.sinh(9.428 * delta_u)
                q_value_correct = q_func(delta_u_correct)
                q_value_wrong = q_func(delta_u_wrong)
                # Let us now focus on the w-function
                w_func = lambda prob: 0.1782 + 0.7186 * math.exp(-1.033 * (-math.log(prob)) ** 0.3705)
                if pr == 0.0:
                    w_value = 0.0
                elif pr == 1.0:
                    w_value = 1.0
